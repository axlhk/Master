...
"This code is optimized for finding the truncation figure as it precomputes in a way not possible in"
"Kitaev_Adiabatic_Aprx_Sparse due to the build of the class."
...
class Kitaev:
    ...
    "trunc_dim:     Where we truncate the sorted coefficients sorted by increasing energy"

    def __init__(self, dt, N, stride, tmax, Hamiltonian, Integration, Evolution, omega_fun, mu_fun, delta_fun, exp_degen, return_full, trunc_dim = None):
    ...
    def precompute_eigensystem_tau(self, Nt_tau):
        ...
        dim_trunc = self.trunc_dim      #trunc_dim = dim by default

        E_store = np.zeros((Nt_tau, dim_trunc))
        V_store = np.zeros((Nt_tau, self.dim, dim_trunc), dtype=complex)
        ...
        for idx, tau in enumerate(tau_grid):
            ...
            if self.ham_builder is H_Kitaev_Chain:
                ...
                H = H_BdG(self.N, t_dummy, 1.0,
                          self.omega_fun, self.mu_fun,
                          self.delta_fun)
            E_full, V_full = self.Diag_H(H)

            # sorter etter energi og behold de trunc_dim laveste
            idx_sorted = np.argsort(E_full)
            keep = idx_sorted[:dim_trunc]

            E_store[idx] = E_full[keep]
            V_store[idx] = V_full[:, keep]

            if 10 and (idx % 10 == 0):
                print(f"[Precompute] Step {idx} / {tau_grid.shape}.")
...
def Eivind_1():
    ...
    for t_, tmax_ in enumerate(tmax_list):
        ...
        T_run, a_t, c_t = model.run_adiabatic_precomputed(T, E_store, V_store)
...
def truncation():
    ...
    # referansemodell (brukes til precompute av eigenpar for tau)
    model_ref = Kitaev(dt=dt_tau,
                       N=N,
                       stride=stride,
                       tmax=1.0,
    ...
    _ = model_ref.build_H(0.0)  # warmup Numba

    print("Precomputing eigenpairs along tau...")
    tau_grid, E_store, V_store = model_ref.precompute_eigensystem_tau(Nt_tau)
    print("Done precomputing.\n")

    plt.figure(figsize=(8, 10))  # 3 rader x 2 kolonner

    all_k_needed = []  # samler alle k over alle tider og tmax

    for t_, tmax_ in enumerate(tmax_list):
        # tidsgitter for denne tmax
        T = tau_grid * tmax_
        dt = T[1] - T[0]
        ...
        if Evolution_type == "Adiabatic":
            # ---------------- Adiabatic_precomputed ----------------
            print(f"Running adiabatic evolution (precomputed) for tmax = {tmax_}...")
            ...
            np.save(f"T_run_{tmax_}.npy", T_run)
            np.save(f"a_t_inst_{tmax_}.npy", a_t_reduced)
            np.save(f"a_t_full_{tmax_}.npy", a_t_full)
            np.save(f"b_t_{tmax_}.npy", b_t)
            np.save(f"E_all{tmax_}.npy", E_all)

            print(f"  Tid total (uten diagonaliseringskostnad per tmax): "
                  f"{t_global_end - t_global_start:.3f} s\n")

            # vi har a_t_full[t_index], E_all[t_index] direkte

            # indekser i tid vi vil se på: start, midt, slutt
            N_a = len(a_t_full)   # antall tidspunkter
            N_i = [0, int(N_a / 2), N_a - 1]

            for i, n_i in enumerate(N_i):
                a_i = a_t_full[n_i]
                E_i = E_all[n_i]

                idx_sorted = np.argsort(E_i)
                E_sorted = E_i[idx_sorted]
                probs_sorted = np.abs(a_i[idx_sorted])**2

                cum = np.cumsum(probs_sorted)
                tail = 1.0 - cum
                k_needed = np.argmax(tail < eps)
                all_k_needed.append(k_needed + 1)

                E0 = E_sorted[0]
                E_rel = E_sorted - E0
                n_idx = np.arange(len(probs_sorted))

                print(f"[Adiabatic] tmax={tmax_}, time index {n_i}, "
                      f"k_needed = {k_needed+1}")

                # rad = i (0,1,2) -> subplot-indeks 2*i+1, 2*i+2
                row = i
                ax1 = plt.subplot(3, 2, 2*row + 1)
                ax1.scatter(n_idx, probs_sorted,
                            alpha=0.7,
                            s=15,
                            label=f"T = {tmax_}, idx={n_i}")
                ax1.set_yscale("log")
                ax1.set_xlabel("tilstandsindeks n (sortert etter energi)")
                ax1.set_ylabel(r"$|a_n|^2$")
                ax1.set_title(f"Amplituder vs indeks (tid idx={n_i})")
                ax1.legend()

                ax2 = plt.subplot(3, 2, 2*row + 2)
                ax2.scatter(E_rel, probs_sorted,
                            alpha=0.7,
                            s=15,
                            label=f"T = {tmax_}, idx={n_i}")
                ax2.set_yscale("log")
                ax2.set_xlabel(r"$E_n - E_0$")
                ax2.set_ylabel(r"$|a_n|^2$")
                ax2.set_title(f"Amplituder vs energi (tid idx={n_i})")
                ax2.legend()

        elif Evolution_type == "Schrodinger":
            # ---------------- Schrödinger ----------------
            print(f"Running Schrödinger evolution for tmax = {tmax_}...")

            t_global_start = time.perf_counter()
            T_run, a_t_reduced, c_t = model.run(T)  # _run_schrodinger
            t_global_end = time.perf_counter()

            print(f"  Tid total: {t_global_end - t_global_start:.3f} s\n")

            # vi har psi(t) = c_t[t_index] i original basis;
            # for utvalgte tidspunkter projiserer vi til egenbasis
            N_a = len(c_t)
            N_i = [0, int(N_a / 2), N_a - 1]
            ...
            for i, n_i in enumerate(N_i):
                ...
                H_i = model.build_H(T_run[n_i])
                E_i, V_i = model.Diag_H(H_i)
                a_i = V_i.conj().T @ psi_i
                ...
                print(f"[Schrödinger] tmax={tmax_}, time index {n_i}, "
                      f"k_needed = {k_needed+1}")
            ...
            raise ValueError(f"Ukjent Evolution_type: {Evolution_type}")

    # global "safe" truncation over alle tider og tmax
    k_safe = max(all_k_needed) if all_k_needed else None
    print("Safe truncation dimension for eps =", eps, "is k_safe =", k_safe)

    plt.tight_layout()
    plt.savefig(f"T={tmax_list}_Truncation_distribution_{Evolution_type}_{Hamiltonian_type}_{N}.pdf")
    plt.show()
# truncation()

def truncation_():
    dt_tau = 5e-3       # steg i tau = t/tmax
    Nt_tau = int(1.0/dt_tau) + 1   # samme for alle tmax
    ...
    # referansemodell for precompute (brukes kun til å hente egenpar langs tau)
    model_ref = Kitaev(dt=dt_tau,
                       N=N,
                       stride=stride,
                       tmax=1.0,
    ...
    for t_, tmax_ in enumerate(tmax_list):
        # tidsgitter for denne tmax (brukes i evolusjon)
        T = np.linspace(0.0, tmax_, Nt_tau)
        dt = T[1] - T[0]
        ...
        if Evolution_type == "Adiabatic":
            # ---------------- Adiabatic_precomputed ----------------
            # Vi vil her også utnytte precompute, så vi bruker samme tau-grid som i precompute:
            # T_tau = tau_grid * tmax_
            T_tau = tau_grid * tmax_
            dt_tau_phys = T_tau[1] - T_tau[0]

            # midlertidig oppdater dt i modellen
            dt_old = model.dt
            model.dt = dt_tau_phys

            print(f"Running adiabatic evolution (precomputed) for tmax = {tmax_}...")

            t_global_start = time.perf_counter()
            # bruker precomputed E_store, V_store direkte
            T_run, a_t_reduced, b_t, a_t_full, E_all = model.run_adiabatic_precomputed(
                T_tau, E_store, V_store
            )
            t_global_end = time.perf_counter()
            ...
            model.dt = dt_old  # reset dt

            # vi har a_t_full[t_index], E_all[t_index] allerede i instantan basis

            N_a = len(a_t_full)
            N_i = [0, int(N_a / 2), N_a - 1]   # start, midt, slutt
            ...
            t_global_start = time.perf_counter()
            T_run, a_t_reduced, psi_t = model.run(T)  # _run_schrodinger
            t_global_end = time.perf_counter()

            np.save(f"T_run_{tmax_}.npy", T_run)
            np.save(f"a_t_inst_{tmax_}.npy", a_t_reduced)
            np.save(f"psi_t_{tmax_}.npy", psi_t)
            ...
            for i, n_i in enumerate(N_i):
                ...
                # bruk precomputed eigenpar ved tilsvarende tau
                tau_i = T_run[n_i] / tmax_           # tau = t / tmax
                idx = np.argmin(np.abs(tau_grid - tau_i))
                E_i = E_store[idx]
                V_i = V_store[idx]

                a_i = V_i.conj().T @ psi_i   # projeksjon til instantan basis
                ...
                mask = tail < eps
                if np.any(mask):
                    k_needed = np.argmax(mask) + 1  # number of states
                else:
                    k_needed = len(probs_sorted)
                all_k_needed.append(k_needed)
...
def adiabatic_approx():
    ...
    # Én modell som vi bruker både til precompute og alle tmax
    model = Kitaev(dt=dt_tau,
                   N=N,
                   stride = stride,
                   tmax=1.0,   # settes om per run
                   Hamiltonian=Hamiltonian_type,
                   Integration=Integrator,
                   Evolution=Evolution_type,
                   omega_fun=omega_Eivind_4_2,
    ...
    # Precompute én gang i tau
    model.prepare_precomputed(Nt_tau)

    plt.figure(figsize=(8,4))

    for tmax_ in tmax_list:
        model.tmax = tmax_  # bestem fysisk totaltid

        print(f"Running Adiabatic_precomputed (trunc={trunc}) for tmax = {tmax_}...")
        t_global_start = time.perf_counter()
        # T_dummy kan være None – ignoreres
        T_run, a_t_reduced, b_t = model.run(T=None)
        t_global_end = time.perf_counter()
        print(f"  Tid: {t_global_end - t_global_start:.3f} s\n")

        tau = T_run / tmax_

        # Sannsynlighet i "adiabatisk gs-subrom" (k=exp_degen)
        P0_adiab = np.sum(np.abs(a_t_reduced)**2, axis=1)

        # Sannsynlighet i første basisvektor i original (truncert) basis
        # (dette er ikke helt identisk med full N=10-grunnstilstand,
        #  men gir et konsistent mål i det truncerte rommet)
        P0_orig = np.abs(b_t[:, 0])**2
