...
class Kitaev:
    ...
    def __init__(self, dt, N, tmax, Hamiltonian, Integration, Evolution,
                 omega_fun, mu_fun, delta_fun, exp_degen):
        ...
        if Integration == "Euler":
            self.stepper = evolution_step_euler
            self.sch_stepper = schrodinger_step_euler
        elif Integration == "Euler_Cromer":
            self.stepper = evolution_step_euler_cromer
            self.sch_stepper = None
        ...
        if Evolution == "Adiabatic":
            # use the *reduced* adiabatic approximation
    ...
    def run(self, T, stride=10):
        return self.run_method(T, stride=stride)

    def Diag_H_full(self, H):
        """Full diagonalization, returns dim eigenpairs."""
        return np.linalg.eigh(H)

    def Diag_H_reduced(self, H):
        """
        Reduced diagonalization: only k = exp_degen lowest eigenpairs.
        For adiabatic *approximation* only.
        """
        k = self.exp_degen
    ...
    def build_H(self, t):
        return self.ham_builder(self.N, t, self.tmax,
    ...
    def run_adiabatic(self, T, stride):
        """
        Adiabatic *approximation* in a k-dimensional low-energy subspace.

        We only keep k = self.exp_degen instantaneous eigenstates of H(t),
        and evolve the amplitudes a(t) in that subspace.

        Returns:
            T, a_t, E_all
        where
            a_t[n]   : length-k amplitudes in reduced adiabatic basis
            E_all[n] : length-k array of instantaneous energies
        """
        dt  = self.dt
        k   = self.exp_degen
        Nt  = len(T)

        a_t = np.zeros((Nt, k), dtype=complex)
        a_t[0, 0] = 1.0

        E_all = np.zeros((Nt, k))
        phase_int = np.zeros((k, k), dtype=complex)

        # t0 and t1
        H0 = self.build_H(T[0])
        E0, V0 = self.Diag_H_reduced(H0)   # (k,), (dim, k)

        H1 = self.build_H(T[1])
        E1_raw, V1_raw = self.Diag_H_reduced(H1)

        V1_matched, perm01 = match_eigenvectors(V0, V1_raw)
        E1 = E1_raw[perm01]
        V1_fixed = fix_phases(V0, V1_matched)
        ...
        # initial A and first step
        A0 = psi_dotpsi_forward(V0, V1_fixed, dt)
        ...
        for n in range(1, Nt - 1):
            ...
            E_next_raw, V_next_raw = self.Diag_H_reduced(H_next)
            ...
            if stride and (n % stride == 0):
                print(f"[Adiabatic reduced] Step {n} / {Nt-1}. T = {self.tmax}")
...
@njit
def H_Kitaev_Chain(N, t, tmax, omega_fun, mu_fun, delta_fun):
    dim = 2**N
    mu = mu_fun(t, tmax)
...
def Eivind_1():
    ...
    "Evolution: Adiabatic or Schrodinger"
    "omega, mu, delta: functions(j, N, t, tmax)" 
    "exp_degen:     Amount of expected degeneracies of the gs"
    ...
    Integrator = "RK4"
    Evolution_type = "Adiabatic"        #Schrodinger is broken in this code
    exp_degen = 4   #Sums up this # of lowest states as the gs

    for t_, tmax_ in enumerate(tmax):   #Loop over all tmax
        dt = dt_ * tmax_    #Using same amount of Nt for each tmax (probably not good for accuracy).
        model = Kitaev(dt=dt,
                        N=N,
                        tmax=tmax_,
        ...
        Nt = int(tmax_/dt) + 1
        T = np.linspace(t0, tmax_, Nt)

        print(f"Running evolution for tmax = {tmax_}...")
        T, a_t, E_T = model.run(T)

        "Saving the data"
        data_a = np.column_stack((T, a_t))

        # np.savetxt(f'adiabatic{Evolution_type}_{Hamiltonian_type}_{Integrator}_{tmax_}_{N}_{dt}_Eivind_4_2.txt', data_a)
        ...
        plt.legend()
        plt.tight_layout()
    plt.savefig(f"{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt}_Eivind_4_2.pdf")
    plt.show()
...
"Gjennskape flere av Eivinds resultater eller se på annet, a.la rotering av kjeder?"
"Eller burde jeg skrive om denne koden først? Gjøre usikkerhets sjekk a.la Eivind? Urelevant for adiabatisk kode"

"Speedup av kode"
"- Oppdatere H istedenfor å bygge den på ny"
"- Mer effektiv diagonalisering? -"
"- Behandle H som en sparse matrise(?) "
"- Kun finne de første egenverdi og vektorene - i.e de degenererte"
