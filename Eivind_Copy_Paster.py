...
def run_and_plot_cut_time_evolution_N10_dtau(
    dtau=1e-2,
    T_list=None,
    N=10,
    mu0=1,
    ...
    save_for_latex=False,
    cutoff_orig=1e-7,
    cutoff_inst=1e-7,
):
    if T_list is None:
        T_list = [0.01, 0.5, 1.0, 5.0]
    ...
    mu_init = mu0 * np.ones(N, dtype=float)
    t_init = t0 * np.ones(N - 1, dtype=float)
    delta_init = delta0 * np.ones(N - 1, dtype=float)

    save_dir = "cut_time_evolution"
    if not os.path.isdir(save_dir):
        os.makedirs(save_dir, exist_ok=True)

    cut_site = N // 2 - 1  # for N=10: 4 → cut between sites 5 and 6

    for T in T_list:
        mu = mu_init.copy()
        t_arr = t_init.copy()
        delta_arr = delta_init.copy()
        ...
        kc.time_evolution(
            ...
            add_energy_constant=False,  # <- IMPORTANT CHANGE
            integrator=integrator,
        )

        name = "cut_time_evolution/N={}_mu0={}_delta0={}_t0={}_ntimesteps={}_neigvals={}_cutsite={}_t={}_onandoff={}".format(
            N,
            mu0,
            delta0,
            t0,
        ...
        KitaevChain.load_and_plot_cut_time_evolution(
            path="",
            file=file,
            title=True,
            save_for_latex=save_for_latex,
            save_path=None,
            cutoff_orig=cutoff_orig,
            cutoff_inst=cutoff_inst,
        )
...
if __name__ == "__main__":
    ...
    run_and_plot_cut_time_evolution_N10_dtau(save_for_latex = True)
