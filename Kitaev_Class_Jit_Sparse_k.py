...
class Kitaev:
    ...
    def __init__(self, dt, N, tmax, Hamiltonian, Integration, Evolution, omega_fun, mu_fun, delta_fun, exp_degen):
...
def Eivind_1():
    ...
    for t_, tmax_ in enumerate(tmax):   #Loop over all tmax
        ...
        T, a_t, c_t = model.run(T)

        "Saving the data"
        data_a = np.column_stack((T, a_t))
        data_b = np.column_stack((T, c_t))

        # np.savetxt(f'a_t{Evolution_type}_{Hamiltonian_type}_{Integrator}_{tmax_}_{N}_{dt}_Eivind_4_2.txt', data_a)
        # np.savetxt(f'c_t{Evolution_type}_{Hamiltonian_type}_{Integrator}_{tmax_}_{N}_{dt}_Eivind_4_2.txt', data_b)
        ...
        # for i in range(shp):
        #     Pi = np.abs(a_t[:, i])**2
        #     plt.plot(tau, Pi, alpha=(shp-i)/shp)

        # plt.plot(tau, norm, label=f"norm {tmax_}", alpha=0.25)
