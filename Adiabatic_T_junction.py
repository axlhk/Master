...
class Kitaev:
    ...
    def build_H(self, t):
        ...
        if self.ham_type == "Kitaev":
            if self.ham_rep == "dense":
                ...
                H = H_Kitaev_sparse_cached_T(
                    self.N, mu, omega_b, delta_b,
                    self.ntot,
                    self.bonds_j, self.bonds_k
                )
        else:
    ...
    def precompute_eigensystem_tau(self, Nt_tau):
        ...
        omega_tab, delta_tab = precompute_couplings_Kitaev_T(
            self.N, tau_grid, 1.0,
            Nb, self.bond_arm, self.bond_depth,
            self.n, self.buffer_sites,
            self.delta_fun, self.omega_fun
        )
    ...
    def precompute_couplings(self, Nt_tau):
        ...
        omega_tab : (Nt_tau, Nb)
        delta_tab : (Nt_tau, Nb)
        where Nb is the number of bonds.
        """
        if self.bonds_j is None:
            raise RuntimeError("Call build_T_geometry() before precompute_couplings().")
        ...
        return tau_grid, omega_tab, delta_tab
...
def H_Kitaev_T(N, t, tmax,
                   omega_fun,  # omega_fun(b, bond_arm[b], bond_depth[b], t, tmax)
                   mu_fun,     # mu_fun(site j, arm/site info if needed, t, tmax)
                   delta_fun,  # delta_fun(b, bond_arm[b], bond_depth[b], t, tmax)
                   ntot_arr,
                   bonds_j, bonds_k,
    ...
    for n_state in range(dim):
        ...
        mu = mu_fun(0, t, tmax)  
        H[n_state, n_state] += - mu * (ntot - N/2)  #mu term
...
@njit
def precompute_couplings_Kitaev_T(N, tau_grid, tmax, Nb,
                                  bond_arm, bond_depth,
                                  n, buffer_sites,
                                  delta_fun, omega_fun):
    Nt = len(tau_grid)
...
def H_Kitaev_sparse_cached_T(N, mu, omega_b, delta_b,
...
#     """
#     Slide a topological segment on arm_target one bond inward:
#       [d_in, d_out]  -->  [d_in-1, d_out-1]

#     s in [0,1] parameterizes the slide:
#       - s=0:  segment is exactly [d_in, d_out].
#       - s in (0, 0.5]: inner bond d_in-1 ramps 0 -> 1, old segment [d_in..d_out] stays 1.
#       - s in (0.5, 1): inner bond d_in-1 is 1, outer bond d_out ramps 1 -> 0.
#       - s=1:  segment is exactly [d_in-1, d_out-1].
#     """
#     if arm != arm_target:
#         return 0.0

#     # Clamp s to [0,1] to be safe
#     if s <= 0.0:
#         # Initial configuration: [d_in, d_out] all 1
...
#     if s >= 1.0:
#         # Final configuration: [d_in-1, d_out-1] all 1
#         if depth >= d_in - 1 and depth <= d_out - 1:
#             return 1.0
#         else:
#             return 0.0

#     # 0 < s < 1
#     # Map the slide into two phases:
#     #   - Phase 1: s ∈ (0, 0.5]  : grow inner bond (d_in-1) 0 -> 1
#     #   - Phase 2: s ∈ (0.5, 1) : shrink outer bond (d_out) 1 -> 0

#     if depth == d_in - 1:
#         # NEW inner bond: 0 -> 1 in Phase 1, stays 1 in Phase 2
#         if s <= 0.5:
#             # smooth ramp 0 -> 1 as s goes 0 -> 0.5
#             x = s / 0.5  # in (0,1]
...
#     if depth == d_out:
#         # OLD outer bond: stays 1 in Phase 1, then 1 -> 0 in Phase 2
#         if s <= 0.5:
#             return 1.0
#         else:
#             # smooth ramp 1 -> 0 as s goes 0.5 -> 1
#             x = (s - 0.5) / 0.5  # in (0,1)
#             return 1.0 - smooth_step(x)

#     if depth > d_in - 1 and depth < d_out:
#         # Bonds strictly between inner and outer are always 1
#         return 1.0

#     # Everything else stays trivial
#     return 0.0
...
def adiabatic_approx_T_junction():
    n = 4
    buffer = 1
    N = 3*n - 2

    "For the cutting of a chain we used 1e-3, or 1/2 e-3 per change (turning on or off). "
    "Moving one arm takes 2(n - 1 + buffer) changes"
    "For move_arm we move 1 arm"
    "For half_circuit we move 3 arms"
    "For full_circle we move 6 arms"
    
    # dt_tau = 1e-3 / (6 * 2 * (n - 1 + buffer))  #Requires 1.07TiB
    # dt_tau = 1e-3 / (2 * (n - 1 + buffer))      #Requires 183GiB
    dt_tau = 1e-3

    # dt_tau = 1/100         #1/1000 took 12962.9s (although pc was "off" when i got back from 2 hour trip so idk if that stopped it, seemed faster)
    ...
    elif protocol == "half_circuit":
        delta_fun = delta_half_circuit
        omega_fun = omega_half_circuit
    ...
    model = Kitaev(dt=dt_tau,
                   ...
                   buffer_sites=buffer)
    model.build_T_geometry()

    _ = model.build_H(0.0)

    model.prepare_precomputed(Nt_tau)

    tau_grid, omega_tab, delta_tab = model.precompute_couplings(Nt_tau)

    np.save(f"T_{protocol}_tau_run_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", tau_grid)
    np.save(f"T_{protocol}_omega_run_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", omega_tab)
    np.save(f"T_{protocol}_delta_run_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", delta_tab)

    x = bond_x_positions(model)             # shape (Nb,)
    np.save(f"T_{protocol}_x_run_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", x)

    plt.figure(figsize=(8,4))

    plt.subplot(1,2,1)
    plt.imshow(omega_tab, aspect='auto', origin='lower',
    ...
    plt.subplot(1,2,2)
    plt.imshow(np.abs(delta_tab), aspect='auto', origin='lower',
            extent=[x.min(), x.max(), tau_grid[0], tau_grid[-1]])
    ...
    plt.tight_layout()
    plt.savefig(f"T_omega_delta_heatmap_{protocol}_{len(tmax_list)}_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamrep}.pdf")
    ...
    for tmax_ in tmax_list:
        ...
        np.save(f"T_{protocol}_T_run_{tmax_}_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", T_run)
        np.save(f"T_{protocol}_a_t_reduced_{tmax_}_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", a_t_reduced)
        np.save(f"T_{protocol}_psi_t_{tmax_}_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", b_t)
        np.save(f"T_{protocol}_c_t_orig_{tmax_}_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamiltonian_type}.npy", P0_orig_phys)
    ...
    plt.tight_layout()
    plt.savefig(f"T_{protocol}_{len(tmax_list)}_{omega0}_{delta0}_{mu0}_{Evolution_type}_{Hamiltonian_type}_{Integrator}_{N}_{dt_tau}_{Hamrep}.pdf")
    plt.show()

if __name__ == "__main__":
    adiabatic_approx_T_junction()

"Maybe i should only vary mu and t instead of delta and t ref. p11 Alicea"

"Appropriate dtau from 1e-3 used when cutting chain"
"Write about code, just lowk write"

"Python time analysis / profiling"

"What to look for"
"- Same final as finish state"
